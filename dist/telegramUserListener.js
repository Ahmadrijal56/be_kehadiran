...
const BOT_USERNAME = process.env.TELEGRAM_MONITOR_BOT_USERNAME?.replace(/^@/, "") ??
    "manjursehatkehadiran_bot";
function isAttendanceText(text) {
    const t = text.trim();
    return t.includes("ID:") || t.includes("NIK:") || t.includes("Status:");
}
async function ingestMessage(message, chatId, source) {
...
async function main() {
    ...
    try {
        ...
        for (const msg of recent.reverse()) {
            if (!msg || !(msg instanceof Object) || !("id" in msg))
                continue;
            const apiMsg = msg;
            if (!apiMsg.message?.trim())
                continue;
...
//# sourceMappingURL=telegramUserListener.js.map