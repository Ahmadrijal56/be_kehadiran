...
export function parseAdmsAttlogLine(line, deviceSn) {
...
function parseVerifyMode(code) {
    const n = parseInt(code, 10);
    if ([15, 16, 17, 20, 21].includes(n))
        return "face_id";
    if (n > 0)
        return "fingerprint";
...
function parseAdmsDateTime(value) {
    ...
    const match2 = normalized.match(/^(\d{2})\/(\d{2})\/(\d{4})\s+(\d{2}):(\d{2})(?::(\d{2}))?$/);
    if (match2) {
...
export function admsLogToVt490Text(log, company = "APT MANJUR SEHAT TSI") {
...
export function parseAdmsAttlogBody(body, deviceSn) {
    const logs = [];
...
//# sourceMappingURL=biofingerAdmsParser.js.map