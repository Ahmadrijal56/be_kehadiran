...
export function resolveScoringMinutes(deltaSeconds, lateThresholdSeconds) {
    const onTime = deltaSeconds <= lateThresholdSeconds &&
        deltaSeconds >= -lateThresholdSeconds;
    if (onTime) {
        return { onTime: true, scoringMinutes: 0 };
    }
    const scoringMinutes = deltaSeconds > lateThresholdSeconds
        ? Math.floor(deltaSeconds / 60)
        : Math.ceil(deltaSeconds / 60);
    return { onTime: false, scoringMinutes };
}
export function calculateKpiScoreFromRules(deltaSeconds, lateThresholdSeconds, rules) {
    const { onTime, scoringMinutes } = resolveScoringMinutes(deltaSeconds, lateThresholdSeconds);
