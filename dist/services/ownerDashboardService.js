...
export async function getOwnerBranchesComparison() {
    ...
    const items = await Promise.all(branches.map(async (branch) => {
        ...
        const presentCount = records.filter((r) => r.status === "present" || r.status === "left")
            .length +
...
export async function getOwnerMonthlyStats(yearMonth) {
    const ym = yearMonth ?? todayWorkDateWib().toISOString().slice(0, 7);
    if (!/^\d{4}-\d{2}$/.test(ym)) {
    ...
    const total_present_days = aggregates.reduce((sum, a) => sum + a.totalPresentDays, 0);
    const total_late_count = aggregates.reduce((sum, a) => sum + a.totalLateCount, 0);
    return {
        year_month: ym,
...
//# sourceMappingURL=ownerDashboardService.js.map