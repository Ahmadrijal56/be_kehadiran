...
function parseHHmm(value) {
    ...
    if (!m)
        throw validationError(`Format jam tidak valid: ${value}`);
    const h = Number(m[1]);
...
async function normalizeBranchWorkShifts(branchId) {
...
export async function ensureBranchShiftsSeeded(branchId) {
...
export async function ensureAllBranchesShiftDefaults() {
...
export async function seedBranchShiftsForNewBranch(branchId) {
    await ensureBranchShiftsSeeded(branchId);
}
export async function getBranchShiftSettings(branchId) {
    await ensureBranchShiftsSeeded(branchId);
    ...
    const shifts = masters.map((master) => {
        ...
        const isWork = WORK_SHIFT_IDS.includes(master.id);
        return {
            shift_id: master.id,
            code: master.code,
            name: master.name,
...
export async function saveBranchShiftSettings(actor, branchId, items) {
    ...
    for (const item of items) {
        ...
        if (!WORK_SHIFT_IDS.includes(item.shift_id) &&
        ...
        const isWork = WORK_SHIFT_IDS.includes(item.shift_id);
        if ((isOff || isWork) && !item.is_active) {
            throw validationError(isOff
        ...
        if (isWork) {
            ...
            if (endMin <= startMin) {
                throw validationError(`Jam selesai harus setelah jam mulai untuk shift ${item.shift_id}`);
            }
        }
    }
    ...
    await prisma.$transaction(items.map((item) => {
        const forceActive = WORK_SHIFT_IDS.includes(item.shift_id) ||
            item.shift_id === OFF_SHIFT_ID;
...
export async function listBranchShiftOptions(branchId) {
    const { shifts } = await getBranchShiftSettings(branchId);
    return shifts
        .filter((s) => s.is_active &&
        (WORK_SHIFT_IDS.includes(s.shift_id) ||
            s.is_off))
        .map((s) => ({
        id: s.shift_id,
        code: s.code,
...
export async function getBranchShiftWindow(branchId, shiftId) {
    await ensureBranchShiftsSeeded(branchId);
...
export async function listBranchShiftDefs(branchId) {
    const { shifts } = await getBranchShiftSettings(branchId);
    return shifts
        .filter((s) => !s.is_off &&
        WORK_SHIFT_IDS.includes(s.shift_id))
...
//# sourceMappingURL=branchShiftConfigService.js.map