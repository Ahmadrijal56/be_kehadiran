import { combineDateAndTimeWib, parseDateTimeDdMmYyyy, parseWorkDateDdMmYyyy, } from "../utils/time.js";
const LABEL_ALIASES = {
    nama: "nama",
    name: "nama",
    nik: "nik",
...
function parseAttendanceType(value) {
    const v = value.trim().toLowerCase();
    if (v.includes("face"))
        return "face_id";
    if (v.includes("finger") || v.includes("sidik"))
        return "fingerprint";
    return undefined;
}
...
function parseVt490Status(value) {
    const v = value.trim().toUpperCase();
    if (v.includes("MASUK"))
        return "masuk";
    if (v.includes("PULANG"))
        return "pulang";
    return undefined;
}
...
function parseVt490Format(fields) {
    const nik = fields.nik?.trim();
    const waktuRaw = fields.waktu?.trim();
    const statusRaw = fields.status?.trim();
    if (!nik)
        throw new Error("PARSER_MISSING_NIK");
    if (!waktuRaw || isEmptyValue(waktuRaw))
        throw new Error("PARSER_MISSING_DATETIME");
    const eventAt = parseDateTimeDdMmYyyy(waktuRaw);
    const workDate = parseWorkDateDdMmYyyy(waktuRaw.match(/^(\d{1,2}\/\d{1,2}\/\d{4})/)?.[1] ?? waktuRaw.split(/\s+/)[0]);
    const result = {
        nama: fields.nama,
        nik,
...
function parseLegacyFormat(fields) {
    const nik = fields.nik?.trim();
    const tanggalRaw = fields.tanggal?.trim();
    if (!nik)
        throw new Error("PARSER_MISSING_NIK");
    if (!tanggalRaw || isEmptyValue(tanggalRaw))
        throw new Error("PARSER_MISSING_DATE");
    ...
    if (fields.istirahatMulai && !isEmptyValue(fields.istirahatMulai)) {
        result.istirahatMulai = combineDateAndTimeWib(workDate, normalizeTime(fields.istirahatMulai));
    }
    if (fields.istirahatSelesai && !isEmptyValue(fields.istirahatSelesai)) {
        result.istirahatSelesai = combineDateAndTimeWib(workDate, normalizeTime(fields.istirahatSelesai));
    }
    return result;
}
/**
...
//# sourceMappingURL=telegramMessageParser.js.map