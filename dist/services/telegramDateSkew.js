 ...
 */
export function correctBiofingerDateSkew(parsed, receivedAt) {
    ...
    if (!anchor)
        return parsed;
    const adjusted = { ...parsed, workDate: today };
    if (parsed.jamMasuk) {
        adjusted.jamMasuk = combineDateAndTimeWib(today, wibClock(parsed.jamMasuk).slice(0, 5));
    }
    if (parsed.jamPulang) {
        adjusted.jamPulang = combineDateAndTimeWib(today, wibClock(parsed.jamPulang).slice(0, 5));
    }
    return adjusted;
}
//# sourceMappingURL=telegramDateSkew.js.map