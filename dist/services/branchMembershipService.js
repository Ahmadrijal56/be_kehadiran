...
export async function listActiveBranchIds() {
...
export async function getBranchIdsForUser(userId, roles) {
...
export async function listBranchesForUser(userId, roles) {
...
export async function ensureUserBranchMembership(userId, branchId) {
...
export async function setUserBranches(userId, branchIds, options) {
    ...
    const primary = options.primaryBranchId && uniqueIds.includes(options.primaryBranchId)
...
export async function assignOwnerToBranch(ownerId, branchId) {
    await ensureUserBranchMembership(ownerId, branchId);
}
export async function moveEmployeeBranch(employeeId, userId, branchId) {
    const branch = await prisma.branch.findFirst({
        where: { id: branchId, isActive: true },
    });
    if (!branch)
        throw validationError("Cabang tidak valid");
    await prisma.$transaction([
...
export function userHasBranchAccess(branchIds, roles, branchId) {
    if (roles.includes("owner"))
        return true;
    return branchIds.includes(branchId);
}
export async function assertBranchesExist(branchIds) {
...
//# sourceMappingURL=branchMembershipService.js.map