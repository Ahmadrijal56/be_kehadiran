...
import { DEFAULT_GAMIFICATION_SETTINGS, DEFAULT_KPI_POINT_RULES, } from "../constants/defaultKpiRules.js";
...
function formatMinuteRange(min, max) {
    const fmt = (m) => {
        const sign = m > 0 ? "+" : m < 0 ? "−" : "";
    ...
    if (min === 0 && max === 0)
        return "+0 menit (tepat jam mulai shift)";
    if (max === null)
        return `> ${fmt(min)} setelah mulai shift`;
    if (min < 0 && max < 0)
        return `${fmt(min)} s/d ${fmt(max)} sebelum mulai shift`;
    if (min <= 0 && max > 0)
        return `${fmt(min)} s/d ${fmt(max)} setelah mulai shift`;
    return `${fmt(min)} s/d ${fmt(max ?? min)}`;
}
function assertCanManageEmployeeTypes(actor) {
...
export async function saveEmployeeTypes(actor, items) {
    ...
    for (const item of items) {
        ...
        if (!item.label?.trim())
            throw validationError(`Label tipe ${code} wajib`);
    ...
    await prisma.$transaction(items.map((item) => prisma.employeeTypeConfig.upsert({
...
export async function listKpiPointRules() {
    await ensureOrganizationDefaults();
...
export async function getActiveKpiRulesCached() {
    const now = Date.now();
    if (rulesCache && now - rulesCacheAt < CACHE_MS)
        return rulesCache;
...
export async function getGamificationSettingsCached() {
    const now = Date.now();
    if (settingsCache && now - rulesCacheAt < CACHE_MS)
...
export async function saveGamificationConfig(actor, data) {
    assertOwner(actor);
    ...
    for (const key of ["top1_amount_idr", "top2_amount_idr", "top3_amount_idr"]) {
        const v = data[key];
    ...
    if (data.kpi_rules) {
        ...
        const incomingIds = new Set(data.kpi_rules.filter((r) => r.id).map((r) => r.id));
...
export async function rewardAmountFromSettings(type) {
    const settings = await getGamificationSettingsCached();
    if (!settings.monthly_rewards_enabled)
        return null;
    if (type === "top_1" || type === "eotm")
        return settings.top1_amount_idr;
    if (type === "top_2")
        return settings.top2_amount_idr;
    if (type === "top_3")
        return settings.top3_amount_idr;
    return null;
}
export async function getPublicRules() {
    ...
    const shiftName = (id) => shifts.find((s) => s.id === id)?.name ?? `Shift ${id}`;
    return {
...
export async function listBranchEmployeesWithType(actor, branchId) {
    if (!actor.roles.includes("owner") &&
        !hasPermission(actor, "users.manage.branch")) {
        throw forbidden();
    }
...
export async function updateEmployeeType(actor, branchId, employeeId, employeeTypeCode) {
    assertCanManageEmployeeTypes(actor);
    assertBranchAccess(actor, branchId);
...
export function shiftAllowedForType(shiftIds, shiftId) {
