...
import { notifyApprovalReviewed, notifyManagersNewApprovalRequest, } from "./notificationService.js";
...
async function ensureAttendanceForDate(employeeId, branchId, workDate) {
...
export async function hasApprovedTwoScanMode(employeeId, workDate) {
...
export async function listMyApprovalRequests(user) {
...
export async function listEligibleApprovalDates(user) {
    const employeeId = requireEmployeeProfile(user);
    ...
    for (let i = 0; i <= LOOKBACK_DAYS; i++) {
        ...
        const pendingOrApproved = new Set(existing
            .filter((e) => e.status === "pending" || e.status === "approved")
            .map((e) => e.type));
        const canSubmit = [];
        if (!pendingOrApproved.has("early_leave"))
            canSubmit.push("early_leave");
        if (!pendingOrApproved.has("no_break"))
            canSubmit.push("no_break");
        if (!pendingOrApproved.has("shift_swap"))
            canSubmit.push("shift_swap");
        if (attendance?.status === "forgot_checkout" &&
    ...
    return dates.filter((d) => d.can_submit_types.length > 0 || d.existing_requests.length > 0);
}
export async function createApprovalRequest(user, data) {
    const employeeId = requireEmployeeProfile(user);
    const employee = await prisma.employee.findUniqueOrThrow({
        where: { id: employeeId },
        select: { branchId: true },
    ...
    const attendance = await ensureAttendanceForDate(employeeId, employee.branchId, workDate);
    ...
    if ((type === "early_leave" || type === "no_break") &&
...
export async function listBranchApprovalRequests(branchId, status) {
...
export async function reviewApprovalRequest(reviewer, requestId, data) {
    const request = await prisma.attendanceApprovalRequest.findUnique({
        where: { id: requestId },
        include: { employee: true, attendance: true },
    });
    if (!request)
        throw notFound("Permintaan tidak ditemukan");
    if (request.status !== "pending") {
        throw businessError("Permintaan sudah diproses");
    }
    if (!userHasBranchAccess(reviewer.branchIds, reviewer.roles, request.branchId)) {
        throw forbidden();
    }
    if (request.type === "shift_swap" && data.status === "approved") {
        throw businessError("Tukar shift: ubah jadwal di halaman Shift terlebih dahulu, lalu konfirmasi perubahan");
    }
    ...
    if (employeeUser) {
        await notifyApprovalReviewed(employeeUser.id, request.type, data.status, requestId, data.manager_note);
    }
    await writeAuditLog({
        userId: reviewer.id,
...
export async function confirmShiftSwapApproval(reviewer, requestId, data) {
    const request = await prisma.attendanceApprovalRequest.findUnique({
        where: { id: requestId },
        include: { employee: true, attendance: true },
    });
    ...
    if (!override) {
        throw businessError("Shift belum diubah. Simpan perubahan jadwal shift terlebih dahulu.");
    }
    if (request.requestedShiftId &&
        override.shiftId !== request.requestedShiftId) {
        throw businessError(`Shift harus diubah ke S${request.requestedShiftId} sesuai permintaan`);
    }
    const updated = await prisma.attendanceApprovalRequest.update({
        where: { id: requestId },
        data: {
    ...
    if (employeeUser) {
        await notifyApprovalReviewed(employeeUser.id, "shift_swap", "approved", requestId, data?.manager_note);
    }
    return mapApprovalRow({ ...updated, employee: request.employee, attendance: request.attendance });
}
export async function rejectApprovalRequest(reviewer, requestId, managerNote) {
    return reviewApprovalRequest(reviewer, requestId, {
        status: "rejected",
        manager_note: managerNote,
    });
}
export async function getApprovalRequest(requestId, user) {
    const request = await prisma.attendanceApprovalRequest.findUnique({
        where: { id: requestId },
        include: {
    ...
    const hasBranch = isOwner ||
        userHasBranchAccess(user.branchIds, user.roles, request.branchId);
    if (!isEmployee && !hasBranch)
        throw forbidden();
    return mapApprovalRow(request);
}
//# sourceMappingURL=attendanceApprovalService.js.map