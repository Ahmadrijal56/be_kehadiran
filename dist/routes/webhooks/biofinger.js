...
import { ingestAdmsLogs, ingestBiofingerRawText, validateBiofingerWebhookSecret, } from "../../services/biofingerIngestService.js";
export const biofingerWebhookRouter = Router();
/** POST JSON { "rawText": "..." } atau text/plain — tanpa chat_id. */
biofingerWebhookRouter.post("/biofinger", async (req, res) => {
    const secret = req.header("x-biofinger-secret") ?? req.header("x-webhook-secret") ?? "";
...
function deviceSn(req) {
    const sn = req.query.SN ?? req.query.sn;
    return typeof sn === "string" ? sn : undefined;
}
biofingerAdmsRouter.all("/cdata", async (req, res) => {
...
biofingerAdmsRouter.get("/getrequest", (_req, res) => {
    res.type("text/plain").send("OK");
});
biofingerAdmsRouter.post("/devicecmd", (_req, res) => {
    res.type("text/plain").send("OK");
});
//# sourceMappingURL=biofinger.js.map