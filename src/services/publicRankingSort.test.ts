...
describe("publicRankingSort", () => {
  it("total poin bulan lebih tinggi selalu di atas", () => {
    const rows = sortAndRankTodayLeaderboard([
      ...
      },
      {
        ...
        total_late_count: 0,
        today_points: 3,
        today_check_in: "06:54",
        today_status: "present",
      },
    ]);

    expect(rows.map((r) => r.nik)).toEqual(["1809", "1802", "1803"]);
    expect(rows.map((r) => r.rank)).toEqual([1, 2, 3]);
  });

  it("total poin sama — absen paling awal di atas", () => {
    const rows = sortAndRankTodayLeaderboard([
      {
        rank: 1,
        nik: "1803",
  ...
  it("compareTodayLeaderboard — belum absen di bawah yang sudah masuk saat poin sama", () => {
    const checked = {
      rank: 0,
      nik: "1",
      full_name: "A",
