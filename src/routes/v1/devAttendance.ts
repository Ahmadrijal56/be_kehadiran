...
async function authorizeManualIngest(
...
devAttendanceRouter.post(
  "/ingest",
  authorizeManualIngest,
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (message.includes("all_slots_filled")) {
        throw businessError(
          "Keempat absen hari ini sudah tercatat (masuk, istirahat mulai, istirahat selesai, pulang)."
        );
      }
      ...
      if (message === "CHECK_IN_ALREADY_RECORDED") {
        throw businessError(
          "Jam masuk sudah tercatat. Scan berikutnya akan diproses sebagai istirahat atau pulang."
        );
      }
