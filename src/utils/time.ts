...
export function timeFromDbTime(value: Date): { hours: number; minutes: number } {
...
export function computeDeltaMinutes(
  ...
  const shiftStart = new Date(
    `${workDateStr}T${String(shift.hours).padStart(2, "0")}:${String(shift.minutes).padStart(2, "0")}:00+07:00`
  );
  const checkInWib = new Date(
    `${workDateStr}T${String(checkHour).padStart(2, "0")}:${String(checkMinute).padStart(2, "0")}:00+07:00`
  );
...
export function computeDeltaSeconds(
  ...
  const checkInWib = new Date(
    `${workDateStr}T${String(get("hour")).padStart(2, "0")}:${String(get("minute")).padStart(2, "0")}:${String(get("second")).padStart(2, "0")}+07:00`
  );
...
export function combineDateAndTimeWib(workDate: Date, hhmm: string): Date {
...
export function parseWorkDateDdMmYyyy(value: string): Date {
  const [dd, mm, yyyy] = value.split("/").map((v) => parseInt(v, 10));
  return new Date(Date.UTC(yyyy, mm - 1, dd));
}

/** Parse DD/MM/YYYY HH:mm[:ss] ke Date WIB. */
export function parseDateTimeDdMmYyyy(value: string): Date {
  ...
  return new Date(
    `${yyyy}-${mm.padStart(2, "0")}-${dd.padStart(2, "0")}T${String(parseInt(h, 10)).padStart(2, "0")}:${m}:${s.padStart(2, "0")}+07:00`
  );
}
